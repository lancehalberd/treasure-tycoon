...
function spellAction(type, action, bonuses, helpText) {
    ...
    action.icon = ifdefor(action.icon, jobIcon(1, 0));
    action.bonuses = bonuses;
    action.helpText = helpText;
    action.tags = ifdefor(action.tags, []);
...
var skills = {
    // Movement actions
    'dodge': movementAction('dodge', {'icon': jobIcon(3, 0)}, {'+cooldown': 10, '+distance': -128, '$buff': buffEffect({}, {'+%evasion': .5, '+duration': 5})},
                            'Leap back to dodge an attack and gain: {$buff}'),
    'sideStep': movementAction('sideStep', {'icon': jobIcon(2, 1), 'rangedOnly': true}, {'+cooldown': 10, '+moveDuration': .05, '+distance': 64,
    ...
    'evadeAndCounter': attackAction('evadeAndCounter', {'icon': jobIcon(3, 0), 'restrictions': ['melee']}, {'$alwaysHits': 'Never misses'}, 'Counter whenever you successfully evade an attack.'),
    'charge': attackAction('charge', {'icon': jobIcon(1, 1), 'tags': ['movement']}, {'+range': 15, '*damage': 2, '+cooldown': 10, '+speedBonus': 3, '+stun': .5,
                                            '+rangeDamage': 0, '$alwaysHits': 'Never misses'},
                                        'Charge at enemies, damaging and stunning them briefly on impact.'),
    'armorBreak': attackAction('attack', {'icon': jobIcon(1, 1), 'restrictions': ['melee']}, {'*damage': 3, '+cooldown': 30, '+stun': .5, '$alwaysHits': 'Never misses',
                                            '$debuff': debuffEffect({}, {'+-armor': ['{strength}', '/', 2], '+-block': ['{strength}', '/', 2]})},
                                            'Deliver a might blow that destroys the targets armor causing: {$debuff}'),
    'blinkStrike': attackAction('attack', {'icon': jobIcon(3, 1), 'restrictions': ['melee'], 'tags': ['movement']}, {'*damage': 1.5, '+cooldown': 6, '$alwaysHits': 'Never misses', '+teleport': 6},
                                'Instantly teleport to and attack a nearby enemy.'),
    'soulStrike': attackAction('attack', {'icon': jobIcon(3, 2), 'restrictions': ['melee']}, {'+range': 2, '*damage': 2, '+cooldown': 15,
    ...
    'snipe': attackAction('attack', {'icon': jobIcon(0, 0), 'restrictions': ['ranged']}, {'+range': 10, '*damage': 2, '+cooldown': 30,
    ...
    'dragonSlayer': attackAction('attack', {'icon': jobIcon(2, 1), 'restrictions': ['melee']},
    ...
    // Generic actions:
    'deflect': genericAction('deflect', {'icon': jobIcon(2, 3)}, {'+damageRatio': [.5, '+', ['{strength}', '/', 100]], '+cooldown': ['20', '*', [100, '/', [100, '+', '{dexterity}']]], '+chance': 1},
                             'Deflect ranged attacks back at enemies.'),
    'plunder': genericAction('plunder', {'icon': jobIcon(2, 3)}, {'+range': 2, '+count': 1, '+duration': ['{strength}', '/', 10], '+cooldown': ['40', '*', [100, '/', [100, '+', '{dexterity}']]]},
                             'Steal an enemies enchantment for yourself.'),
    'distract': genericAction('dodge', {'icon': jobIcon(3, 0)}, {'$globalDebuff': debuffEffect({}, {'+*accuracy': .5, '+duration': 2}), '+cooldown': 10},
    ...
    'pet': genericAction('minion', {'icon': jobIcon(4, 2), 'target': 'none', 'tags': ['minion'], 'monsterKey': 'wolf'}, {'+limit': 1, '+cooldown': 30},
    ...
    'net': genericAction('effect', {'icon': jobIcon(0, 0)}, {'+cooldown': 10, '+range': 10, '$debuff': debuffEffect({}, {'+*speed': 0, '+duration': 3})},
                         'Throw a net to ensnare a distant enemy.'),
    'sicem': genericAction('effect', {'icon': jobIcon(4, 2)}, {'+cooldown': [60, '*', [100, '/', [100, '+', '{dexterity}']]],
    ...
    'smokeBomb': genericAction('criticalCounter', {'icon': jobIcon(4, 1)}, {'$dodgeAttack': true, '+cooldown': 100, '$globalDebuff': debuffEffect({},
    ...
    'attackSong': genericAction('song', {'icon': jobIcon(2, 0), 'tags': ['song', 'field'], 'target': 'allies', 'color': 'orange', 'alpha': .2},
    ...
    'defenseSong': genericAction('song', {'icon': jobIcon(2, 0), 'tags': ['song', 'field'], 'target': 'allies', 'color': 'purple', 'alpha': .2},
    ...
    'heroSong': genericAction('heroSong', {'icon': jobIcon(2, 0), 'tags': ['song', 'field'], 'target': 'allies', 'color': 'gold', 'alpha': .2},
    ...
    'raiseDead': genericAction('minion', {'icon': jobIcon(1, 0), 'target': 'enemies', 'targetDeadUnits': true, 'consumeCorpse': true, 'tags': ['spell']},
                               {'+limit': 10, '+range': 10, '+chance': .4, '+cooldown': .5},
                                'Chance to raise defeated enemies to fight for you.'),
    'tomFoolery': genericAction('dodge', {'icon': jobIcon(0, 2)}, {'+cooldown': 30, '$buff': buffEffect({}, {
                '+*accuracy': 0, '$$maxEvasion': 'Evasion checks are always perfect', '+duration': 5})},
                'Dodge an attack and gain: {$buff}'),
    'mimic': genericAction('mimic', {'icon': jobIcon(0, 2)}, {}, 'Counter an enemy ability with a copy of that ability.'),
    'decoy': genericAction('decoy', {'icon': jobIcon(0, 2), 'tags': ['minion']}, {'+cooldown': 60},
    ...
    'dispell': spellAction('spell', {'icon': jobIcon(1, 0), 'tags': ['blast'], 'height': 20, 'color': 'grey', 'alpha': .4},
